...
class Api extends REST_Controller
{
    ...
    function insertRespuesta_get(){
        if(!$this->get('Tiempo') && !$this->get('Estado') && !$this->get('RutAlumno') && !$this->get('idActividad'))
        {
            $this->response(NULL, 400);
        }
        $msn = $this->GestionModel->insertRespuesta($this->get('Tiempo'),$this->get('Estado'),$this->get('RutAlumno'),$this->get('idActividad'));
        if($msn){
