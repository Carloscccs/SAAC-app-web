	...
	*/
	...
	$.fn.CSVToTable = function(csvFile, options) {
		var defaults = {
			...
			tdClass: "",
			loadingImage: "",
			loadingText: "Cargando archivo CSV...",
			separator: "\t",
			startLine: 0,
			tipo: 0
		};	
			...
			$.get(csvFile, function(data) {
					...
					if ((lineCount == options.startLine) && (typeof(options.headers) == 'undefined')) {
						...
						if (items.length > 1) {
							...
							if (items.length != headerCount) {
								error += 'error en línea ' + lineCount + ': Item  (' + items.length + ') no coincide con el encabezado (' + headerCount + ') \n';
							}
							(printedLines % 2) ? oddOrEven = 'odd' : oddOrEven = 'even';
							//tableHTML += '<tr class="' + options.trClass + ' ' + oddOrEven + '">';
							$.each(items, function(itemCount, item) {
								//item = item.replace("o","\u00f3");}
								/*var sal = '';
								item = unescape(encodeURIComponent(item));
								for (var i = 0;i<item.length;i++){
									if(item.charCodeAt(i)==239){
										i+=3;
									}
									if(i<item.length)
										sal +=item.charAt(i);
								}*/
								tableHTML +='<input type="hidden" id="'+lineCount+itemCount+'" value="'+item+'"/>'
								//tableHTML += '<td class="' + options.tdClass + '">'+item+'<input type="hidden" id="'+lineCount+itemCount+'" value="'+item+'"/></td>';
							});
							//tableHTML += '</tr>';
				...
				if (error) {
					...
					obj.fadeOut(500, function() {
						...
						setTimeout(function() {
							obj.trigger("Carga Completa");
							if(options.tipo == 1){
								listaCargaDocentes(alto,ancho);
							}
							if(options.tipo == 2){
								listaCargaAsignaturas(alto,ancho);
							}
							if(options.tipo == 3){
								listaCargaEvaluacionDocente(alto,ancho);
							}
							if(options.tipo == 4){
								listaCargaAsignaturas2(alto,ancho);
							}
							if(options.tipo == 5){
								listaCargaIndicadorDocente(alto,ancho);
							}
							if(options.tipo == 6){
								listaCargaVariabilidadDeCarga(alto,ancho);
							}
							if(options.tipo == 7){
								listaCargaSemaforo(alto,ancho);
							}
						},0);
					});
				}
